...
- `fsl.html` — Pagina dedicata alla FSL (Formazione Scuola Lavoro): descrizione del contesto, le due fasi dell'esperienza, competenze acquisite, riflessione personale.
...
# Preferenze visive — vincolanti

Non scegliere palette, font e mood in autonomia: usa esattamente le indicazioni qui sotto, che sostituiscono la libertà totale prevista dalla sezione "Stile visivo" del prompt.

## Palette (variabili CSS da dichiarare in `:root`)
```css
:root {
  --color-bg-dark:          #1e2832;
  --color-bg-light:         #f6f1e9;
  --color-bg-card:          #ffffff;
  --color-accent:           #c07d4a;
  --color-text-on-dark:     #f6f1e9;
  --color-text-muted-dark:  #8a9aa8;
  --color-detail-dark:      #c0b89a;
  --color-text-on-light:    #1e2832;
  --color-text-muted-light: #4a5560;
}
```

## Tipografia
- Titoli e nome: **Playfair Display** (Google Fonts), pesi 400 e 700, anche italic.
- Corpo e UI: **Lato** (Google Fonts), pesi 300 / 400 / 700.
- Non usare Inter, Roboto o font di sistema.

## Mood
Editoriale italiano — rivista curata, non portfolio da developer. Caldo ma sobrio. Niente neon, niente parallax, niente effetti aggressivi.

## Alternanza sezioni
Hero → `--color-bg-dark` · Chi sono → `--color-bg-light` · Passioni → `--color-bg-dark` · Materie → `--color-bg-light` · Footer → `#141c24`.

## Regole accent
`--color-accent` solo per: label eyebrow in maiuscoletto, CTA primario, linea decorativa sotto l'hero, hover degli elementi interattivi. Mai come background di interi blocchi.

## Bottoni
`border-radius: 0` su tutti i bottoni. CTA primario: background `--color-accent`, testo `--color-bg-light`. CTA secondario: background trasparente, bordo sottile `--color-detail-dark`.

## Navigazione
Sticky, background `--color-bg-dark`, logo in Playfair Display, link in maiuscoletto con `letter-spacing`.

## Hero (`index.html`)
Due colonne: testo a sinistra (≈70%), foto profilo circolare a destra (≈30%). Mobile: colonna unica, foto sopra. Struttura testo: eyebrow maiuscoletto `--color-accent` → nome Playfair ≥48px desktop (cognome in italic) → sottotitolo maiuscoletto grigio → descrizione Lato Light → due CTA.

## Pagina FSL
Il file si chiama `fsl.html` (non `pcto.html`). Il titolo della pagina e la voce nel menù di navigazione sono "FSL" o "Formazione Scuola Lavoro", mai "PCTO".

## Animazioni
Fade-in sobrio sulle sezioni allo scroll tramite IntersectionObserver. Nessun altro effetto automatico.
